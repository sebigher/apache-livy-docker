...
RUN cd /tmp && \
        wget http://mirrors.hostingromania.ro/apache.org/spark/spark-2.4.7/spark-2.4.7-bin-hadoop2.7.tgz && \
        tar zxvf spark-2.4.7-bin-hadoop2.7.tgz -C /usr/local --owner root --group root --no-same-owner && \
        rm spark-2.4.7-bin-hadoop2.7.tgz
# Rename Spark folder
RUN cd /usr/local && ln -s spark-2.4.7-bin-hadoop2.7 spark
...
RUN wget http://mirror.evowise.com/apache/incubator/livy/0.7.0-incubating/apache-livy-0.7.0-incubating-bin.zip
